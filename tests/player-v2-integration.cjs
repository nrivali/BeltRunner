...
(async()=>{
 ...
 const browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',args:['--enable-webgl','--use-angle=swiftshader','--enable-unsafe-swiftshader']});const results=[];const crypto=require('node:crypto'),gameFile=path.join(root,'belt-runner-3d.html'),sha=b=>crypto.createHash('sha256').update(b).digest('hex'),gameSHA=sha(fs.readFileSync(gameFile));
 try{
  for(const mode of ['http','file','script-fallback','missing','invalid']){
   ...
   if(mode==='script-fallback')await page.route('**/player_ship.glb',r=>r.fulfill({status:404,body:'missing'}));
   ...
   await page.goto(url+'?debug',{waitUntil:'load',timeout:60000});await page.waitForFunction(()=>window.BeltRunner?.playerAsset.state && BeltRunner.playerAsset.state!=='loading',null,{polling:100,timeout:60000});
   await page.waitForFunction(()=>BeltRunner.carrierAsset.state!=='loading',null,{polling:100,timeout:60000});
   const result=await page.evaluate(()=>({state:BeltRunner.playerAsset.state,error:BeltRunner.playerAsset.error,revision:BeltRunner.playerAsset.revision,source:BeltRunner.playerAsset.source}));assert.equal(result.state,mode==='missing'||mode==='invalid'?'fallback':'ready');
   if(mode!=='missing'&&mode!=='invalid'){
    ...
    Object.assign(result,await page.evaluate(()=>{
     ...
     for(const wings of ['delta','swept','forward','twin'])for(let laser=0;laser<=4;laser++)for(let cargo=0;cargo<=5;cargo++)for(let engine=0;engine<=4;engine++)for(let scanner=0;scanner<=3;scanner++){
      ...
      let triangles=0;api.ship.traverseVisible(o=>{if(o.isMesh)triangles+=(o.geometry.index?o.geometry.index.count:o.geometry.attributes.position.count)/3;});if(triangles>24000)throw new Error('Budget exceeded');maxTriangles=Math.max(maxTriangles,triangles);refits++;
     }
    ...
    if(mode==='http'){
     await page.evaluate(()=>{const b=BeltRunner;document.querySelectorAll('body > :not(canvas):not(script)').forEach(e=>e.style.display='none');const target=b.ship.pos.clone();b.camera.position.copy(target).add(new THREE.Vector3(70,42,95).applyQuaternion(b.ship.q));b.camera.lookAt(target);b.lighting.update({camera:b.camera,station:b.station,quality:'high',dt:.1});b.renderer.render(b.scene,b.camera);});
     await page.screenshot({path:path.join(root,'assets/player_ship_v2/previews/in-game.png')});
    }
   ...
   assert.deepEqual(errors,[]);results.push({mode,...result,errors});console.log('PASS '+mode);await page.close();
  }
  assert.equal(sha(fs.readFileSync(gameFile)),gameSHA,'Shared game changed during test');fs.writeFileSync(path.join(root,'assets/player_ship_v2/integration-validation.json'),JSON.stringify({gameSHA256:gameSHA,assetSHA256:sha(fs.readFileSync(path.join(root,'assets/player_ship_v2/player_ship.glb'))),results},null,2));console.log(JSON.stringify(results,null,2));
