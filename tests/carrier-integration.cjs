 ...
 try{
  for(const mode of ['http','file','script-fallback','missing','invalid']){
   ...
   const result=await page.evaluate(()=>{
    ...
    return {state:a.state,error:a.error,revision:a.revision,source:a.source,triangles:a.triangles,meshes:a.meshes,dishMeshes:a.dishMeshes,totalTriangles:a.totalTriangles,model:!!model,engines:b.stationEngines.map(e=>e.position.toArray()),pivot:b.TURRET_PIVOT.toArray(),pads:b.DEPOT.bays.map(v=>v.parkL.toArray()),floor:b.DEPOT.bays[0].y0,fields:b.forceFields.map(f=>f.mesh.parent===b.station),lights:b.stationPointLights.length,anchors:a.anchors&&Object.keys(a.anchors).length,normalMaps:a.materials&&Object.values(a.materials).every(m=>m.normalMap?.image.width===2048),drop:b.DROP_PAD.toArray(),playerReady:b.playerAsset.state==='ready'};
   });
   const fallback=mode==='missing'||mode==='invalid';
   assert.equal(result.state,fallback?'fallback':'ready',JSON.stringify(result));
   assert.ok(requests.every(u=>u.includes('/cargo_carrier_v2/')));assert.ok(result.playerReady);
   if(!fallback){
    assert.equal(result.revision,2);assert.equal(result.triangles,35316);assert.equal(result.totalTriangles,37496);assert.equal(result.meshes,4);assert.equal(result.dishMeshes,3);assert.equal(result.anchors,13);assert.ok(result.normalMaps);
    ...
    result.rig=await page.evaluate(()=>{
     ...
     rock.pos.copy(world([2000,2400,2500]));b.state.depot.laser=3;Object.assign(b.depotLaser,b.turretAnglesTo(rock.pos),{rock,retarget:1});
     b.updateDepot(0);b.updateVisuals(0,2);b.scene.updateMatrixWorld(true);
     ...
     rock.pos.copy(saved.pos);rock.hp=saved.hp;b.state.depot.laser=saved.level;Object.assign(b.depotLaser,{rock:null,firing:false,yaw:0,pitch:.15});b.updateDepot(0);b.updateVisuals(0,3);b.scene.updateMatrixWorld(true);
    ...
    if(mode==='http'){
     ...
     result.warp=await page.evaluate(()=>{
      const b=BeltRunner,model=b.station.getObjectByName('cargo_carrier_model'),id=model.uuid,destination=b.ZONES.find(z=>z!==b.zone&&!z.hub);
      b.warpTo(destination);for(let i=0;i<220;i++)b.update(.05);b.updateVisuals(0,12);b.scene.updateMatrixWorld(true);
      if(b.zone!==destination||!b.flags.docked||model.uuid!==id||model.parent!==b.station)throw new Error('Carrier lost during warp');
     ...
     const shot=async name=>{const temp=path.join(root,'assets/cargo_carrier_v2/previews',name+'.tmp.png');await page.screenshot({path:temp});fs.renameSync(temp,path.join(root,'assets/cargo_carrier_v2/previews',name+'.png'));};
     await shot('in-game');
