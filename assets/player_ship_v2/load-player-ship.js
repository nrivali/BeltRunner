window.loadBeltRunnerPlayerShip=async function({renderer,install,status}){
  try{
    ...
    const url=new URL('assets/player_ship_v2/player_ship.glb',document.baseURI);let data,source='glb';
    ...
    }catch(fetchError){source='script';
    ...
    install(api);Object.assign(status,{state:'ready',revision:2,source,error:null,components:api.parts.size});return api;
