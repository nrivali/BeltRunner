...
window.loadBeltRunnerCarrier = async function ({station, renderer, glowTexture, install, status}) {
  ...
  try {
    ...
    const url=new URL('assets/cargo_carrier_v3/cargo_carrier_assembled.glb',document.baseURI);
    let data,source='glb';
    ...
    const hullShape=JSON.parse(model.getObjectByName('hull_collision_profile')?.userData.definition||'null');
    ...
    if(meshes!==4||dishMeshes!==3||triangles+dishTriangles>65000)throw new Error('Unexpected carrier mesh layout');
    ...
    Object.assign(status,{state:'ready',revision:3,source,triangles,meshes,dishTriangles,dishMeshes,totalTriangles:triangles+dishTriangles,anchors,materials,hullShape,error:null});
    return model;
  } catch(error) {
