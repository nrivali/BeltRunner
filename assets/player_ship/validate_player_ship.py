...
assert all(abs(a-b)<1e-5 for a,b in zip(nodes['dish_mount']['translation'],[0,-4.5,7]))
...
for im in doc['images']:
    ...
    assert struct.unpack_from('>II',binary,offset+16)==(1024,1024)
...
assert max(report['assembled_triangle_counts'].values())<=8000
...
for level in (1,2,3):
    ...
    for degrees in range(-90,91,5):
        ...
        sweeps+=1
    yaw.rotation_euler.z=0;bpy.context.view_layer.update();barrel.parent=saved_parent;barrel.matrix_world=world;bpy.context.view_layer.update()
result={'status':'PASS','required_nodes':len(required),'assembled_triangle_range':[min(report['assembled_triangle_counts'].values()),max(report['assembled_triangle_counts'].values())],'all_components_triangles':report['triangles_all_components'],'radius':radius,'texture_size':1024,'embedded_pngs':4,'uv_overlap_count':len(collisions),'yaw_clearance_samples':sweeps,'yaw_range_degrees':[-90,90],'camera_or_lights_in_glb':False,'scene_count':2}
